...
class CNN(nn.Module):
    def __init__(self, target_number):
        ...
        self.number_targets = 2#len(targets[target_number])
        ...
        self.network = nn.Sequential(
            ...
            nn.Conv1d(4, 8, kernel_size=500, stride=10, padding=0),
            nn.BatchNorm1d(8),
            nn.ReLU(inplace=True),
            ...
            nn.Linear(32, self.number_targets)
            )
    def forward(self, x):
        x = self.network(x)
        return x
...
class CustomImageDataset(torch.utils.data.Dataset):
    def __init__(self, x, y, target_number, transform=None, target_transform=None, stop=0):
        ...
        self.len_img = 7998
        targets = [range(256)]
        self.target_array = [0, 1]#np.array(targets[target_number])
        ...
        self.y = torch.tensor(np.load(y)[:,0], dtype=torch.int64)[:self.stop]
    ...
    def __getitem__(self, idx):
        ...
        label = int(label)#int(np.where(self.target_array == float(label))[0][0])
        #print(label)
        if self.transform:
...
def train(model, device, train_loader, optimizer, epoch, noise_amplitude=0, bit=0):
    '''
    train the model
    '''
    model.train()
...
def test(model, device, test_loader, plot = False, bit = 0):
    '''
    test the model
    '''
    model.eval()
    ...
    error = 0
    if plot:table = np.zeros((256, 2))
    with torch.no_grad():
        for data, target in test_loader:
            #print("Test number " + str(counter))
            if plot: real_target = target 
            target = (target % 2**(bit+1)) >= 2**bit
            target = target.type(torch.int64)
            ...
            if plot :
                for i in range(len(target)):
                    table[int(real_target[i]), pred[i][0]] += 1
    ...
    if plot:
        print(table)
        maximums = table.argmax(axis=1)
        print(maximums)
        bit_target = (np.array(range(256)) % 2**(bit+1)) >= 2**bit
        print(bit_target)

        print("max accuracy = ", np.sum(maximums == bit_target)/2.56, ' %')
        # np.save("table.npy", table)
        # correct = 0
        # for tab in table:
        #     if tab == [[True, False], [False, True]]:
        #         correct += 1
