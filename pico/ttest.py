...
class ttest(object):
    ...
    def ttest_trsfile(self, traceset, disable_tqdm=False):
        ...
        print("t-value MAX : ", max(self.tstat))#, -min(self.tstat)))
        block0 = self.t_var[0]**2/self.Nt[0]
        block1 = self.t_var[1]**2/self.Nt[1]
        self.d_freedom = (block0 + block1)**2 / (block0**2/(self.Nt[0]-1) + block1**2/(self.Nt[1]-1) )
        print("degrees of freedom :", self.d_freedom[self.tstat.argmax()])
        return self.tstat
