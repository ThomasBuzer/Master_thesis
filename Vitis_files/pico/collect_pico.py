...
def collect(app, n_traces, model):
    ...
    if COLLECT_TRACES:
        ...
        traceFileName = "traces/" + model + "_" + "{}_{}".format(n_traces, current_time) + ".trs"
    ...
    triggerChannel = 4
    voltRangeTrigger = 5000 * 1e-3 #V/div
    threshold = 1000 * 1e-3 #V
    ratio_pre_trigger=0
    trigger_delay = 1.5* 1e-3 #s
    ...
    if COLLECT_TRACES:
        ...
        scope.setTriggerChannel(triggerChannel,enable=1, delay=trigger_delay, timeout=10000)#, threshold=threshold, timeout=10000, ratio_points_pre_trigger=ratio_pre_trigger, delay=trigger_delay) #4 is Ext Channel
