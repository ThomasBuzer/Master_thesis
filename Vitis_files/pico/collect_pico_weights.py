...
def ssh_thread(app, n_traces, coin):
    out = ssh.execute_command("python3 "+PATH+app+" -c "+str(coin)+" --image_dir "+PATH+"images/ -n "+str(n_traces), return_rc=False, return_stdout=True, output_during_execution=True)
...
def collect(app, n_models):
    ...
    if COLLECT_TRACES:
        ...
        traceFileName = "traces/" + "weights/trace" + "_" + "{}_{}".format(n_models, current_time) + ".trs"
        #commFileName = "traces/" + "Xoodyak_FVR" + "{}_{}".format(n_traces, current_time) + ".txt"
        #commFile = open(commFileName, "w")
    else:
        n_models = 10e9 #Set big number of traces to keep computation going when setting parameters in the picoscope GUI.
    ...
    if COLLECT_TRACES:
        headers = {
            ...
            trsfile.Header.TRACE_PARAMETER_DEFINITIONS:definitions,
        ...
        traceFile = trsfile.trs_open(traceFileName, mode='w', headers=headers, padding_mode = trsfile.TracePadding.AUTO)
    ...
    print("Entering loop")
    # Main loop
    n_traces_per_model = 5
    for numTotal in tqdm(range(int(n_models)), ascii=True, desc='Capture'):
        ...
        ssh_th=threading.Thread(target=ssh_thread, args=[app, n_traces_per_model, coin])
        ssh_th.start()
        #time.sleep(2)
        ...
        for j in range(n_traces_per_model):
            ...
            try:
                ...
                if COLLECT_TRACES:
                    ...
                    traceFile.append(trsfile.Trace(trsfile.SampleCoding.BYTE, channel_out, parameters=parameters))# + data_output))
                    #print("Pico ready !")                
...
def run_main():
    ...
    ap.add_argument('-a', '--app',   type=str,  default='app_server_weight.py',       help='App.py to use on the ZCU board. Default is app_server_big_picture.py')
    ap.add_argument('-n', '--n_models',   type=str,  default=1,       help='Number of models to capture. Default is 1')
    args = ap.parse_args()
    ...
    collect(args.app, args.n_models)
    return
